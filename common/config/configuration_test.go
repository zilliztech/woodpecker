...
func TestNewConfiguration(t *testing.T) {
	...
	assert.Equal(t, 4, config.Woodpecker.Logstore.LogFileSyncPolicy.MaxFlushThread)
	...
	assert.Equal(t, 8, defaultConfig.Woodpecker.Logstore.LogFileSyncPolicy.MaxFlushThread)
...
func TestNewWoodpeckerConfiguration(t *testing.T) {
	...
	assert.Equal(t, 100000, config.Logstore.LogFileSyncPolicy.MaxEntries)
	assert.Equal(t, 64000000, config.Logstore.LogFileSyncPolicy.MaxBytes)
	assert.Equal(t, 5, config.Logstore.LogFileSyncPolicy.MaxFlushRetries)
	assert.Equal(t, 1000, config.Logstore.LogFileSyncPolicy.RetryInterval)
	assert.Equal(t, 8000000, config.Logstore.LogFileSyncPolicy.MaxFlushSize)
	assert.Equal(t, 4, config.Logstore.LogFileSyncPolicy.MaxFlushThread)
	...
	assert.Equal(t, 3, defaultConfig.Logstore.LogFileSyncPolicy.MaxFlushRetries)
	assert.Equal(t, 2000, defaultConfig.Logstore.LogFileSyncPolicy.RetryInterval)
	assert.Equal(t, 16000000, defaultConfig.Logstore.LogFileSyncPolicy.MaxFlushSize)
	assert.Equal(t, 8, defaultConfig.Logstore.LogFileSyncPolicy.MaxFlushThread)
}
