...
import (
	...
	"github.com/zilliztech/woodpecker/common/generic"
...
func (pool *Pool[T]) Resize(size int) error {
	if pool.opt.preAlloc {
		return errors.New("cannot resize pre-alloc pool")
	}
	if size <= 0 {
		return errors.Newf("positive size:%s", strconv.FormatInt(int64(size), 10))
	}
