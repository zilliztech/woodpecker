...
func IsSegmentNotWritableErr(err error) bool {
	...
	return ErrSegmentHandleSegmentClosed.Is(err) || ErrSegmentFenced.Is(err) || ErrStorageNotWritable.Is(err) || ErrFileWriterFinalized.Is(err) || ErrFileWriterFinalizing.Is(err) || ErrWoodpeckerClientClosed.Is(err)
}
