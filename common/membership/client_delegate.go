...
// nothing of its own, but it does keep a ServiceDiscovery and select on load, so
// it has to ingest the load announces servers gossip.
type ClientDelegate struct {
	// discovery receives peer load readings via NotifyMsg. Optional; nil disables
	// ingestion. Issue #271.
...
// current load — without it a client would select on whatever load each server
// happened to publish when the client joined. Unknown tags are ignored.
func (d *ClientDelegate) NotifyMsg(buf []byte) {
	...
	case msgTypeLoadUpdate:
		...
		applyLoadUpdate(d.discovery, buf[1:])
	}
}
