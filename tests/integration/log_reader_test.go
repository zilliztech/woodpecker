...
func TestActiveSegmentRead(t *testing.T) {
	...
	mockSegmentHandle.EXPECT().RefreshAndGetMetadata(mock.Anything).Return(&proto.SegmentMetadata{
		State:       proto.SegmentState_Active,
		LastEntryId: int64(-1), // it is active, not completed yet
	})
	...
	logReader := log.NewLogReader(ctx, mockLogHandle, mockSegmentHandle, log.EarliestLogMessageID)
...
func TestSegmentInExceptionState(t *testing.T) {
	...
	mockSegmentHandle1.EXPECT().RefreshAndGetMetadata(mock.Anything).Return(&proto.SegmentMetadata{
		SegNo:       int64(1),
		State:       proto.SegmentState_Active,
		LastEntryId: int64(-1), // it is active, not completed yet
	})
...
func TestReadFromEarlyNotExistsPoint(t *testing.T) {
	...
	mockSegmentHandle2.EXPECT().RefreshAndGetMetadata(mock.Anything).Return(&proto.SegmentMetadata{
		State:       proto.SegmentState_Active,
		LastEntryId: int64(-1), // it is active, not completed yet
	})
	msg0 := &log.WriterMessage{
	...
	ctx := context.Background()
	logReader := log.NewLogReader(ctx, mockLogHandle, nil, log.EarliestLogMessageID)
	msg, readErr := logReader.ReadNext(ctx) // read entry#0
	assert.NoError(t, readErr)
