...
func TestInitIfNecessary(t *testing.T) {
	...
	err := etcd.InitEtcdServer(true, "", "/tmp/TestInitIfNecessary", "/tmp/TestInitIfNecessary.log", "info")
...
func TestCreateLogAndOpen(t *testing.T) {
	// init&start etcd server
	err := etcd.InitEtcdServer(true, "", "/tmp/TestCreateLogAndOpen", "/tmp/TestCreateLogAndOpen.log", "info")
	assert.NoError(t, err)
	defer etcd.StopEtcdServer()
	etcdCli, err := etcd.GetEtcdClient(true, false, []string{}, "", "", "", "")
	assert.NoError(t, err)
...
func TestStoreQuorumInfo(t *testing.T) {
	// init&start etcd server
	err := etcd.InitEtcdServer(true, "", "/tmp/TestStoreQuorumInfo", "/tmp/TestStoreQuorumInfo.log", "info")
	assert.NoError(t, err)
	defer etcd.StopEtcdServer()
	etcdCli, err := etcd.GetEtcdClient(true, false, []string{}, "", "", "", "")
	assert.NoError(t, err)
...
func TestStoreSegmentMeta(t *testing.T) {
	// init&start etcd server
	err := etcd.InitEtcdServer(true, "", "/tmp/TestStoreSegmentMeta", "/tmp/TestStoreSegmentMeta.log", "info")
	assert.NoError(t, err)
	defer etcd.StopEtcdServer()
	etcdCli, err := etcd.GetEtcdClient(true, false, []string{}, "", "", "", "")
	assert.NoError(t, err)
	...
	// test get segmentMeta
	{
		...
		assert.Equal(t, segmentMeta.SegNo, segmentMeta.SegNo)
		assert.Equal(t, segmentMeta.Offset, segmentMeta.Offset)
	}
