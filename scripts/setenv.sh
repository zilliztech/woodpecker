...
case "${unameOut}" in
    Linux*)
      # check if use asan.
      MILVUS_ENABLE_ASAN_LIB=$(ldd $ROOT_DIR/external/output/lib/libmilvus_core.so | grep asan | awk '{print $3}')
      ...
      LIBJEMALLOC=$PWD/external/output/lib/libjemalloc.so
      if test -f "$LIBJEMALLOC"; then
        export LD_PRELOAD="$LIBJEMALLOC"
      else
        echo "WARN: Cannot find $LIBJEMALLOC"
      fi
      export PKG_CONFIG_PATH="${PKG_CONFIG_PATH}:$ROOT_DIR/external/output/lib/pkgconfig:$ROOT_DIR/external/output/lib64/pkgconfig"
      export LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:$ROOT_DIR/external/output/lib:$ROOT_DIR/external/output/lib64"
    ...
    Darwin*)
      ...
      export PKG_CONFIG_PATH="${PKG_CONFIG_PATH}:$ROOT_DIR/external/output/lib/pkgconfig"
      export DYLD_LIBRARY_PATH=$ROOT_DIR/external/output/lib
      export RPATH=$DYLD_LIBRARY_PATH;;
    MINGW*)
      extra_path=$(cygpath -w "$ROOT_DIR/external/output/lib")
