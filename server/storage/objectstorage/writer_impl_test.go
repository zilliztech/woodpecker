...
func newTestMinioFileWriter() *MinioFileWriter {
	...
	w := &MinioFileWriter{
		...
		fastSyncTriggerSize: syncPolicyConfig.MaxFlushSize,
	...
	w.lastModifiedTime = time.Now().UnixMilli()

	newBuffer := cache.NewSequentialBuffer(1, 0, 0, maxBufferEntries)
...
func buildValidBlockData(t *testing.T, blockID int64, firstEntryID int64, payloads [][]byte) []byte {
	...
	entries := make([]*cache.BufferEntry, 0, len(payloads))
	for i, payload := range payloads {
		entries = append(entries, &cache.BufferEntry{
			EntryId: firstEntryID + int64(i),
			Data:    payload,
		})
	}
	return w.serialize(blockID, entries)
}
...
func TestMinioFileWriter_PrepareMultiBlockData_MultiBlock(t *testing.T) {
	...
	w := newTestMinioFileWriter()
	w.syncPolicyConfig.MaxFlushSize = 5
...
func TestMinioFileWriter_FastFlushFailUnsafe(t *testing.T) {
	...
	blockData := []*cache.BufferEntry{
		{EntryId: 0, Data: []byte("data0"), NotifyChan: resultCh1},
		{EntryId: 1, Data: []byte("data1"), NotifyChan: resultCh2},
	}
...
func TestMinioFileWriter_ReleaseSegmentLock_Success(t *testing.T) {
	...
	mockClient.EXPECT().RemoveObject(mock.Anything, "test-bucket", "test-base/1/0/write.lock").
		Return(nil).Once()
...
func TestMinioFileWriter_IsSegmentLocked_NotLocked(t *testing.T) {
	...
	notFoundErr := werr.ErrEntryNotFound.WithCauseErrMsg("not found")
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", "test-base/1/0/write.lock").
...
func TestMinioFileWriter_CreateSegmentLock_Success(t *testing.T) {
	...
	mockClient.EXPECT().PutObjectIfNoneMatch(mock.Anything, "test-bucket", "test-base/1/0/write.lock", mock.Anything, mock.Anything).
		Return(nil).Once()
...
func TestMinioFileWriter_Compact_AlreadyCompacted(t *testing.T) {
	...
	mockClient.EXPECT().WalkWithObjects(mock.Anything, "test-bucket", mock.Anything, false, mock.Anything).
		Return(nil).Maybe()
...
func TestMinioFileWriter_Compact_ReadBlockError(t *testing.T) {
	ctx := context.Background()
	mockClient := mocks_objectstorage.NewObjectStorage(t)
	w := newTestMinioFileWriter()
	w.client = mockClient
	...
	readErr := errors.New("read block failed")
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", mock.Anything).
		Return(int64(0), false, readErr).Maybe()

	_, err := w.Compact(ctx)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "read block failed")
}
...
func TestMinioFileWriter_RecoverFromStorage_NoFooter_NoBlocks(t *testing.T) {
	...
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", "test-base/1/0/footer.blk").
		Return(int64(0), false, notFoundErr).Once()
	...
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk").
		Return(int64(0), false, notFoundErr).Once()
...
func TestMinioFileWriter_RecoverFromFullListing_WithBlocks(t *testing.T) {
	...
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk", int64(0), readSize).
		Return(mockReader, nil).Once()

	notFoundErr := werr.ErrEntryNotFound.WithCauseErrMsg("not found")
	// Block 1 does not exist
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", "test-base/1/0/1.blk").
		Return(int64(0), false, notFoundErr).Once()
	mockClient.EXPECT().IsObjectNotExistsError(notFoundErr).Return(true).Maybe()
...
func TestMinioFileWriter_RecoverFromFooter(t *testing.T) {
	...
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", footerBlockKey, int64(0), footerBlockSize).
		Return(mockReader, nil).Once()
...
func TestMinioFileWriter_RecoverFromFooter_GetObjectError(t *testing.T) {
	...
	footerBlockKey := "test-base/1/0/footer.blk"
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", footerBlockKey, int64(0), int64(100)).
...
func TestMinioFileWriter_RecoverFromFooter_TooSmall(t *testing.T) {
	...
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", footerBlockKey, int64(0), int64(len(smallData))).
		Return(mockReader, nil).Once()
...
func TestMinioFileWriter_Finalize_Success(t *testing.T) {
	...
	mockClient.EXPECT().PutObjectIfNoneMatch(mock.Anything, "test-bucket", mock.Anything, mock.Anything, mock.Anything).
		Return(nil).Maybe()
...
func TestMinioFileWriter_DeleteOriginalBlocksByKeys_Success(t *testing.T) {
	...
	mockClient.EXPECT().RemoveObject(mock.Anything, "test-bucket", mock.Anything).
		Return(nil).Times(2)
...
func TestMinioFileWriter_DeleteOriginalBlocksByKeys_PartialFailure(t *testing.T) {
	...
	mockClient.EXPECT().RemoveObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk").
		Return(nil).Once()
	mockClient.EXPECT().RemoveObject(mock.Anything, "test-bucket", "test-base/1/0/1.blk").
...
func TestMinioFileWriter_ListOriginalBlockFiles(t *testing.T) {
	...
	mockClient.EXPECT().WalkWithObjects(mock.Anything, "test-bucket", "test-base/1/0/", false, mock.Anything).
		Run(func(ctx context.Context, bucketName string, prefix string, recursive bool, fn commonObjectStorage.ChunkObjectWalkFunc) {
...
func TestMinioFileWriter_ReadBlockData_Success(t *testing.T) {
	...
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", blockKey).
		Return(int64(len(blockContent)), false, nil).Once()
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", blockKey, int64(0), int64(len(blockContent))).
...
func TestMinioFileWriter_ReadBlockData_GetError(t *testing.T) {
	...
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", blockKey).
		Return(int64(100), false, nil).Once()
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", blockKey, int64(0), int64(100)).
		Return(nil, errors.New("get error")).Once()
...
func TestMinioFileWriter_UploadSingleMergedBlock_FirstBlock(t *testing.T) {
	...
	mockClient.EXPECT().PutObject(mock.Anything, "test-bucket", "test-base/1/0/m_0.blk", mock.Anything, mock.Anything).
		Return(nil).Once()
...
func TestMinioFileWriter_UploadSingleMergedBlock_NonFirstBlock(t *testing.T) {
	...
	mockClient.EXPECT().PutObject(mock.Anything, "test-bucket", "test-base/1/0/m_1.blk", mock.Anything, mock.Anything).
		Return(nil).Once()
...
func TestMinioFileWriter_Compact_FullFlow(t *testing.T) {
	...
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk", int64(0), int64(len(block0Data))).
		Return(&writerMockFileReader{data: block0Data}, nil).Once()

	// readBlockData for block 1
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", "test-base/1/0/1.blk").
		Return(int64(len(block1Data)), false, nil).Once()
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", "test-base/1/0/1.blk", int64(0), int64(len(block1Data))).
	...
	mockClient.EXPECT().PutObject(mock.Anything, "test-bucket", "test-base/1/0/footer.blk", mock.Anything, mock.Anything).
		Return(nil).Once()
...
func TestMinioFileWriter_Fence_ConditionWriteSuccess(t *testing.T) {
	...
	mockClient.EXPECT().PutFencedObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk").
		Return(nil).Once()
...
func TestMinioFileWriter_Fence_WithExistingBlocks(t *testing.T) {
	...
	// Fence at block 2
	mockClient.EXPECT().PutFencedObject(mock.Anything, "test-bucket", "test-base/1/0/2.blk").
		Return(nil).Once()
...
func TestNewMinioFileWriterWithMode_RecoveryMode_WithExistingData(t *testing.T) {
	...
	mockClient.EXPECT().StatObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk").
		Return(int64(len(blockData)), false, nil).Once()
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk", int64(0), readSize0).
...
func TestMinioFileWriter_ParseBlockHeaderRecord_Block1(t *testing.T) {
	...
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", "test-base/1/0/1.blk", int64(0), readSize).
		Return(&mockFileReader{data: headerData}, nil).Once()
...
func TestMinioFileWriter_Fence_Success_WithExistingBlocks(t *testing.T) {
	...
	mockClient.EXPECT().PutFencedObject(mock.Anything, "test-bucket", "test-base/1/0/1.blk").
		Return(nil).Once()
...
func TestMinioFileWriter_CleanupOriginalFilesIfCompacted_WithOriginalFiles(t *testing.T) {
	...
	mockClient.EXPECT().WalkWithObjects(mock.Anything, "test-bucket", "test-base/1/0/", false, mock.Anything).
		Run(func(ctx context.Context, bucketName string, prefix string, recursive bool, walkFunc commonObjectStorage.ChunkObjectWalkFunc) {
...
func TestMinioFileWriter_WriteDataAsync_BufferFullTriggerSync(t *testing.T) {
	...
	newBuffer := cache.NewSequentialBuffer(1, 0, 0, 3)
	w.buffer.Store(newBuffer)
...
func TestMinioFileWriter_Finalize_SyncError(t *testing.T) {
	...
	mockClient.EXPECT().PutObjectIfNoneMatch(mock.Anything, "test-bucket", "test-base/1/0/footer.blk", mock.Anything, mock.Anything).
		Return(nil).Once()
...
func TestMinioFileWriter_Fence_RetryWithRecovery(t *testing.T) {
	...
	mockClient.EXPECT().GetObject(mock.Anything, "test-bucket", "test-base/1/0/0.blk", int64(0), block0ReadSize).
...
func TestMinioFileWriter_RecoverFromFooter_ParseFooterError(t *testing.T) {
	...
	minFooterSize := codec.RecordHeaderSize + codec.FooterRecordSize
	...
	err := w.recoverFromFooter(ctx, footerBlockKey, footerBlockSize)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to decode footer record")
}
...
func TestMinioFileWriter_FlushingBufferSize_AccountingAccuracy(t *testing.T) {
	...
	assert.Equal(t, int64(-152), finalSize)
}
