...
func TestNewFragmentObject(t *testing.T) {
	...
	fragment := NewFragmentObject(client, "test-bucket", int64(8*1024*1024), 1, "test-key", [][]byte{[]byte("entry1"), []byte("entry2")}, 100, true, false, true)
...
func TestFragmentObject_Flush_EmptyFragment(t *testing.T) {
	client := mocks_minio.NewMinioHandler(t)
	fragment := NewFragmentObject(client, "test-bucket", int64(8*1024*1024), 1, "test-key", [][]byte{}, 100, false, false, false)
	err := fragment.Flush(context.Background())
	assert.Error(t, err)
...
func TestFragmentObject_Load(t *testing.T) {
	client := mocks_minio.NewMinioHandler(t)
	fragment := NewFragmentObject(client, "test-bucket", int64(8*1024*1024), 1, "test-key", [][]byte{}, 100, false, true, false)
	data := make([]byte, 0)
...
func TestMergeFragmentsAndReleaseAfterCompleted(t *testing.T) {
	...
	fragment1 := NewFragmentObject(client, "test-bucket", int64(8*1024*1024), 1, "test-key1", [][]byte{[]byte("entry1")}, 100, true, false, true)
	fragment2 := NewFragmentObject(client, "test-bucket", int64(8*1024*1024), 2, "test-key2", [][]byte{[]byte("entry2")}, 101, true, false, true)
