...
func TestSegmentProcessor_Compact(t *testing.T) {
	...
	mockLogFile.EXPECT().Merge(mock.Anything).Return([]storage.Fragment{
		objectstorage.NewFragmentObject(mockMinio, "test-bucket", uint64(0), "woodpecker/1/1/0/m_0.frag", [][]byte{[]byte("data0"), []byte("data1"), []byte("data2")}, int64(10), true, false),
		objectstorage.NewFragmentObject(mockMinio, "test-bucket", uint64(1), "woodpecker/1/1/0/m_1.frag", [][]byte{[]byte("data3"), []byte("data4"), []byte("data5")}, int64(13), true, false),
...
func TestSegmentProcessor_Recover(t *testing.T) {
	...
	mockLogFile.EXPECT().Load(mock.Anything).Return(
		int64(0),
		objectstorage.NewFragmentObject(mockMinio, "test-bucket", uint64(1), "woodpecker/1/1/0/1.frag", [][]byte{[]byte("data3"), []byte("data4"), []byte("data5")}, int64(13), true, false),
		nil)
	cfg := &config.Configuration{
		Minio: config.MinioConfig{
			BucketName: "test-bucket",
