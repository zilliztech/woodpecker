...
func TestLogStore_EvictLog_RejectsServing(t *testing.T) {
	...
	_, err := store.EvictLog(ctx, testBucketName, testRootPath, testLogId, false)
	assert.NoError(t, err)
...
func TestLogStore_EvictLog_Idempotent(t *testing.T) {
	...
	_, firstErr := store.EvictLog(context.Background(), testBucketName, testRootPath, testLogId, false)
	require.NoError(t, firstErr)
	_, secondErr := store.EvictLog(context.Background(), testBucketName, testRootPath, testLogId, false)
...
func TestLogStore_EvictLog_PersistsMarker(t *testing.T) {
	...
	_, evictErr := store.EvictLog(context.Background(), testBucketName, testRootPath, testLogId, false)
...
func TestLogStore_EvictLog_MarkerWriteFailure(t *testing.T) {
	...
	_, err := store.EvictLog(context.Background(), testBucketName, testRootPath, testLogId, false)
	require.Error(t, err)
